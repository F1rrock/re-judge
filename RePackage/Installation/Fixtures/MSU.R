tryCatch(
  {
    ...
    {
      required <- c(
        "dotenv", "httr", "xml2", "rvest", "jsonlite",
        "rstudioapi"
      )
    ...
    {
      path <- normalizePath(file.path(getwd(), "RePackage"),
        winslash = "/",
        mustWork = TRUE
      )
    ...
    }
    {
      ...
      system2("R", args = c(
        "CMD", "INSTALL", "--no-clean-on-error",
        "--no-staged-install", path
    ...
    }
    {
      ...
      contents(text.required(
        text.fstring(
          ...
          "Open Addins → Change credentials,", "to verify your settings."
        ),
        text.then(
  ...
  error = function(e) {
    cat(paste0(
      "\f", "\033[31m", "Error: ", conditionMessage(e),
      "\033[39m"
    ))
  }
)
